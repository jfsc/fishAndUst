...
public class StringUtils {
    ...
    public static boolean containsAllIgnoreCase(String str, String ... args) {
    	String strmin = str.toLowerCase();
    	for (String arg : args) {
			if ( !strmin.contains(arg.toLowerCase()) ) return false;
		}
    	return true;
    }
	...
	/**
	 * <p>Indicate if exist the minimum one occurrence <code>char</code> on the given text</p>
	 * @param name str to be checked
	 * @param chars occurrences <code>char[]</code>
	 * @return true if at least one occurrence exist
	 * @author rafaelrocha
	 */
	public static boolean containsAnyIgnoreCase(String name, char[] chars) {
		String min = name.toLowerCase();
		for (char shar : chars) {
			if ( min.contains(String.valueOf(shar)) ) return true;
		}
		return false;
	}
	
	/**
	 * <p>Returns the substring of filename if the filename length is  greater than limit size</p>
	 * @param filename
	 * @param screenLimit
	 * @param proportion
	 * @return substring of filename
	 * @author amanajás
	 */
	public static String getLimitedFileName(String filename, int screenLimit)
	{
		...
		float nameProportion = (count * 10); 
		while(nameProportion >= screenLimit)
		{
			filename = filename.substring(0, count) + "...";
			count--;
			nameProportion = (count * 10); 
		}
	...
	/**
	 * <p>Returns if the server message is an error</p>
	 * @param String resp
	 * @return boolean
	 * @author amanajás
	 */

	public static boolean hasError(String resp)
	{
		if(resp != null && resp.length() > 0)
		{
			return resp.toLowerCase().contains(Constants.SERVER_SEARCH_FOR_ERROR);
		}
		return false;
	}
